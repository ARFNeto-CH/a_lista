...
Node* inserir_node_antes (void*, Node*);
Node* inserir_node_depois(void*, Node*);
...
int         define_maximo(Lista* l, const unsigned m)
{
    if (l == NULL) return -1;
...
Lista*      inserir_na_ordem(void* item, Lista* l, int(*f)(void*, void*))
{
    ...
    do
    {
        ...
        if (res == 0)
            ...
            if (p == l->fim)
                l->fim = inserir_node_depois(item, p);
            else
                p = inserir_node_depois(item, p);
        else
        {
            // aqui tem uma pegadinha se for inserir antes do primeiro
            if (p == l->inicio)
                l->inicio = inserir_node_antes(item, p);
            else
                p = inserir_node_antes(item, p);
        };  // if()
...
Lista*      inserir_no_inicio(void* item, Lista* l)
{
    if (l == NULL) return l; // sem lista
...
Lista*      inserir_no_final(void* item, Lista* l)
{
    if (l == NULL) return l;
    if ((l->maximo > 0) && (l->quantos == l->maximo)) return l; // nao cabe
...
int         listar_do_seu_jeito(Lista* l, int(*f)(void*))
{
    if (l == NULL) return -1;
    if (l->maximo > 0)
    {
...
Lista*      remover(void* item, Lista* l)
{
    return NULL;
};  // remover()
...
Node*       inserir_node_antes(void* item, Node* deste)
{
...
Node*       inserir_node_depois(void* item, Node* deste)
{
    // era pra retornar se desse erro no malloc()
    Node* novo = (Node*)malloc(sizeof(Node));
    novo->item = (Node*)item; // o dado em si
